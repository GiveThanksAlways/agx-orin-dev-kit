#!/usr/bin/env python3
"""YOLOv8 live camera demo — C Hot Path dispatch on Jetson AGX Orin.

Maximum-speed variant: after @TinyJit warmup, the GPU command queue is
exported and replayed from C via raw MMIO doorbell writes.  Zero Python
overhead in the inference hot loop.

Preprocessing is done on CPU (numpy/OpenCV letterbox), then the float32
NCHW tensor is memcpy'd into the static GPU input buffer.  The C code
patches timeline/kickoff values, submits via GPFifo, waits for
completion, then we read the output buffer.
...
═══════════════════════════════════════════════════════════════
  ...
  # Build hot_path.so (first time only):
  make
...
═══════════════════════════════════════════════════════════════
  ...
  # Standard (C hot path, size 320):
  NV=1 python3 demo_yolov8_hot_path.py --stream --size 320

  # With BEAM kernel auto-tuning (slower warmup, faster kernels):
  JITBEAM=2 NV=1 python3 demo_yolov8_hot_path.py --stream --size 320

  # All-out fastest:
  JITBEAM=4 NV=1 python3 demo_yolov8_hot_path.py --stream --size 320

═══════════════════════════════════════════════════════════════
 HOW IT WORKS
═══════════════════════════════════════════════════════════════

  1. Load YOLOv8 model in tinygrad
  2. Create static input/output GPU buffers
  3. Warmup with @TinyJit → captures HCQGraph
  4. Export graph config (buffer addrs, GPFifo, patches)
  5. Load hot_path.so (C shared library)
  6. Camera loop:
     a. CPU letterbox + normalize frame → float32 NCHW
     b. C: memcpy input → patch cmdq → GPFifo doorbell → wait → memcpy output
     c. Python: postprocess detections → annotate → stream/save

═══════════════════════════════════════════════════════════════
 COMPARISON: demo_yolov8_camera.py vs demo_yolov8_hot_path.py
═══════════════════════════════════════════════════════════════

  demo_yolov8_camera.py    — Python @TinyJit dispatch (~12 FPS @ 320)
  demo_yolov8_hot_path.py  — C MMIO doorbell dispatch (faster)

  The C hot path eliminates Python dispatch overhead by replaying
  the exact same GPU commands from C.  For the small-model control
  loop benchmark (18K MLP), this gives 2.3x speedup (106µs → 46µs).
  For YOLOv8 (much heavier GPU work), the speedup is smaller but
  still meaningful — the dispatch overhead is constant while GPU
...
═══════════════════════════════════════════════════════════════
  ...
  1. On the Orin:   NV=1 python3 demo_yolov8_hot_path.py --stream
  2. On laptop:     ssh -L 9999:localhost:8090 Orin-AGX-NixOS
  3. Browser:       http://localhost:9999/
"""
...
def setup_hot_path(model, imgsz, warmup_count):
    """Load model, warmup JIT, export graph, return C dispatch function.

    Returns: (c_dispatch, out_shape, dev, cfg_struct, lib)
      c_dispatch(input_np) → output_np   (one C GPU iteration)
    """
    dev = Device["NV"]
    ...
    print("JIT warmup complete — exporting HCQ graph for C hot path...")

    # Export graph config using expression-based sym classification
    ...
    # Build and init the C config struct
    ...
    _pinned_refs = (static_x, static_o, x_buf, o_buf, jit_run)

    def c_dispatch(input_np):
        """Run one GPU inference iteration via C hot path.

        Args: input_np — contiguous float32 (1,3,H,W)
        Returns: (output_np, time_ns)
        """
        _ = _pinned_refs  # prevent GC
    ...
    return c_dispatch, out_shape, dev, cfg_struct, lib
...
def run_bench(args):
    ...
    c_dispatch, out_shape, dev, cfg_struct, lib = setup_hot_path(
